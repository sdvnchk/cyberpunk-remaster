...
requireMaintenanceWrite("Импорт предметов из стороннего компендия");
...
try {
  ...
  const target = path.join(root, "content", "exports", "items.json");
  const current = JSON.parse(await fs.readFile(target, "utf8"));
  const importedById = new Map(imported.map((item) => [item._id, item]));
  const merged = current.map((item) => importedById.get(item._id) ?? item);
