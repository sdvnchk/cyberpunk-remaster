...
requireMaintenanceWrite("Добавление исходного RED-контента");
...
const ITEM_PREFIX = "Compendium.cyberpunk-remaster.cyberpunk-items.Item";
...
const equipmentTemplate = structuredClone(itemById.get("6Lnce5KtpF8VqE6O"));
const meleeTemplate = structuredClone(itemById.get("krNr5PTr5HmccpGz"));
const rangedTemplate = structuredClone(itemById.get("rgprAOXrCIFtwfpr"));
...
function makeEquipment({
  ...
  item.system.description = {
    gm: "",
    value:
      (/<(?:p|hr|ul|ol|table)\b/i.test(effect) ? effect : `<p>${effect}</p>`) +
...
const universalItems = [
  ...
  makeEquipment({
    ...
    moduleFlags: {
      ...
      grantItemUuids: [`${ITEM_PREFIX}.${IDS.chronosFocus}`],
    },
  }),
  makeEquipment({
    ...
    moduleFlags: {
      exclusiveFamily: "cyberdeck",
      grantItemUuids: [`${ITEM_PREFIX}.${IDS.akashaFocus}`],
    },
  }),
  makeEquipment({
    ...
    hardCost: 3,
    implantType: "Модуль",
    effect: `Предиктивная дека сопоставляет телеметрию противника с библиотекой атак, вредоносных программ и отказов техники. Модуль устанавливается в Нейролинк. Пока дека установлена, она добавляет @UUID[${ITEM_PREFIX}.${IDS.oracleFocus}]{Контрпрогноз «Оракул машин»} в ваши фокусные программы и увеличивает максимум Очков Фокуса на 1. Одновременно можно установить только одну кибердеку.`,
    moduleFlags: {
      exclusiveFamily: "cyberdeck",
      grantItemUuids: [`${ITEM_PREFIX}.${IDS.oracleFocus}`],
    },
  }),
  makeEquipment({
    id: IDS.aerohypo,
...
const uniqueItems = [
  ...
  makeEquipment({
    ...
    moduleFlags: {
      grantItemUuids: [`${ITEM_PREFIX}.${IDS.kildareProtocol}`],
    },
  }),
  makeEquipment({
...
const enhancedHearingRule = enhancedHearing?.system?.rules?.find(
  (rule) =>
    ...
    (rule.selector === "perception" || rule.selector?.includes?.("perception")),
);
if (enhancedHearingRule) {
  enhancedHearingRule.predicate = ["action:seek", "item:trait:auditory"];
}
  ...
  twins.choices = [
    {
      key: "appearance",
      choose: 1,
      itemIds: ["Ndx3eGnbRw5bYDLj", "mqkOCr9ufgYYDUto"],
      stress: "normal",
      locked: true,
    },
  ];
...
function pageContent(current) {
  ...
  const choiceRows = choices
    .map((choice) => {
      ...
      return `<p><strong>Выберите ${choice.choose}:</strong> ${options}</p>`;
    })
    .join("");
...
function normalizeJournalTraitSections(content) {
  ...
  while (innermost.test(normalized)) {
    ...
    normalized = normalized.replace(innermost, (_match, body) => {
      ...
      return traits.length ? `<p class="traits">${traits.join(" ")}</p>` : "";
    });
    innermost.lastIndex = 0;
  }
  return normalized;
...
const rows = models.map((current) => {
  ...
  return {
    ...
    components:
      all.reduce(
        (sum, component) => sum + Math.max(1, Number(component.quantity) || 1),
        0,
      ) +
