...
const GRADE_ALIASES = new Map(
  Object.keys(WEAPON_UPGRADES).map((grade) => [grade, grade]),
);
...
function itemGrade(item) {
  ...
  for (const trait of traits) {
    const grade = GRADE_ALIASES.get(String(trait).toLocaleLowerCase("en"));
    if (grade) return grade;
  }
  const value = item.system?.grade?.value ?? item.system?.grade;
  return GRADE_ALIASES.get(String(value ?? "").toLocaleLowerCase("en")) ?? null;
}
...
function injectUpgradeBadge(html, item) {
  ...
  if (!root) return;
  root.querySelectorAll(".sf2eu-overlay").forEach((element) => element.remove());
  appendUpgradeBadge(
...
function observeActorUpgradeSummaries(app, html) {
  ...
  actorUpgradeObservers.get(app)?.disconnect();
  ...
  actorUpgradeObservers.set(app, observer);
}
...
Hooks.once("setup", () => {
  if (!registerHumanityRuleElement()) {
    console.error(
      `${MODULE_ID} | SF2e Rule Element API is unavailable.`,
    );
  }
