...
export async function createNeoArchiveController(hostRoot, { requestClose = async () => {} } = {}) {
  ...
  function entryTypeWidePanels(book, entry) {
    ...
    if ( entry.type === "clues" ) {
      return `${detailValue("Версия", entry.theory, {wide:true})}${detailValue("Что подтвердилось", entry.conclusion, {wide:true})}${clueConnectionsRead(book, entry)}${locationPanel}`;
    }
    if ( entry.type === "books" ) {
    ...
    return `<div class="pcm-detail pcm-record-console" data-entry-id="${entry.id}">
      ...
      <div class="pcm-record-matrix">
        ...
        <div class="pcm-record-lower-grid">
          <div class="pcm-detail-grid pcm-data-panels pcm-record-wide-data">${entryTypeWidePanels(book, entry)}${directoryContactsPanel(book, entry)}${worldMapLinksPanel(entry)}${readFragments(entry)}</div>
