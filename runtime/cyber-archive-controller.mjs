 ...
 */
export async function createCyberArchiveController(hostRoot, { requestClose = async () => {} } = {}) {

  const VARIANT = "cyber-archive-view";
  const MACRO_VERSION = "7.1.0";
  ...
  const CONTACT_TYPE_META = {
    gang: { label: "Банда", section: "gangs", icon: "✦", legacyField: "gang" },
    corporate: { label: "Корпорат", section: "corporations", icon: "▰", legacyField: "corporation" },
    fixer: { label: "Фиксер", section: "fixers", icon: "◆", legacyField: "fixer" },
    ripper: { label: "Рипер", section: "rippers", icon: "✚", legacyField: "ripper" },
    lawman: { label: "Законник", section: "lawmen", icon: "⚖", legacyField: "lawGroup" },
    noosphere: { label: "Ноосфера", section: "noosphere", icon: "⌁", legacyField: "noosphereGroup" },
    nomad: { label: "Кочевник", section: "nomads", icon: "⌂", legacyField: "nomadGroup" }
  };
  ...
  const THEME_GROUPS = {
    calm: { label: "Спокойные", icon: "◌", hint: "Для долгого чтения и многочасовой сессии: минимум зрительного шума." },
    corp: { label: "Корпорации", icon: "▰", hint: "Сдержанные фирменные палитры крупнейших сил Найт-Сити." },
    gang: { label: "Банды / кланы", icon: "✦", hint: "Более характерные и выразительные темы, но без кислотного фона под текстом." },
    city: { label: "Службы и город", icon: "⌁", hint: "Полиция, медики, Сеть, бары и ночные улицы." }
  };
  const THEME_PRESETS = {
    graphite: { label: "Мягкий графит", group: "calm", background: "#151b1d", panel: "#222a2c", text: "#e7e4dc", muted: "#a5aaa6", accent: "#c9bd97", secondary: "#7fa8a4", opacity: 0.98, density: "comfortable" },
    rain: { label: "Дождливый город", group: "calm", background: "#141b20", panel: "#20292f", text: "#dfe7e8", muted: "#99a5a9", accent: "#8fb7be", secondary: "#aa91a8", opacity: 0.97, density: "comfortable" },
    concrete: { label: "Тёплый бетон", group: "calm", background: "#211e1d", panel: "#2d2927", text: "#ece6de", muted: "#aaa09a", accent: "#c49d7f", secondary: "#859f9d", opacity: 0.98, density: "comfortable" },
    moss: { label: "Серо-зелёная", group: "calm", background: "#171d1a", panel: "#242c27", text: "#e3e7df", muted: "#9fa79f", accent: "#9fb28f", secondary: "#8fa7ad", opacity: 0.98, density: "comfortable" },
    smoke: { label: "Светлый дым", group: "calm", background: "#d7dbd8", panel: "#ecefed", text: "#29312e", muted: "#66706c", accent: "#607d76", secondary: "#8a7064", opacity: 0.99, density: "comfortable" },
    cream: { label: "Тёплая бумага", group: "calm", background: "#e3ddd2", panel: "#f3eee5", text: "#37312b", muted: "#746c63", accent: "#8c6c52", secondary: "#647d79", opacity: 0.99, density: "comfortable" },

    arasaka: { label: "Arasaka — Чёрный лак", group: "corp", background: "#111315", panel: "#1b1e21", text: "#ece9e6", muted: "#aaa7a3", accent: "#b94a52", secondary: "#b8a58f", opacity: 0.98, density: "comfortable" },
    militech: { label: "Militech — Стальная олива", group: "corp", background: "#151914", panel: "#232920", text: "#e8eadf", muted: "#a6ad9b", accent: "#9aa86e", secondary: "#b37a65", opacity: 0.98, density: "comfortable" },
    kangtao: { label: "Kang Tao — Нефритовый контур", group: "corp", background: "#101a19", panel: "#1d2927", text: "#e4ece8", muted: "#99aaa5", accent: "#5fa99b", secondary: "#d1a55f", opacity: 0.98, density: "comfortable" },
    biotechnica: { label: "Biotechnica — Лабораторный мох", group: "corp", background: "#151c17", panel: "#253027", text: "#e6ece5", muted: "#a3ada3", accent: "#7ea77d", secondary: "#88a9ad", opacity: 0.98, density: "comfortable" },
    nightcorp: { label: "Night Corp — Синий час", group: "corp", background: "#121824", panel: "#20293a", text: "#e5eaf2", muted: "#9faabd", accent: "#718bb8", secondary: "#9b82ad", opacity: 0.98, density: "comfortable" },
    petrochem: { label: "Petrochem — Индустриальная охра", group: "corp", background: "#201b14", panel: "#30281d", text: "#eee8dd", muted: "#b0a590", accent: "#b28b54", secondary: "#7d9b8c", opacity: 0.98, density: "comfortable" },
    sovoil: { label: "SovOil — Тёмная нефть", group: "corp", background: "#171314", panel: "#261f20", text: "#eee5e4", muted: "#ab9d9d", accent: "#a65f62", secondary: "#b39268", opacity: 0.98, density: "comfortable" },
    zetatech: { label: "Zetatech — Холодный интерфейс", group: "corp", background: "#11191d", panel: "#1d2930", text: "#e5edf0", muted: "#9caab0", accent: "#6ca4b7", secondary: "#8d8bb4", opacity: 0.98, density: "comfortable" },

    tygerclaws: { label: "Tyger Claws — Розовый неон", group: "gang", background: "#19141a", panel: "#2a202b", text: "#f0e6ee", muted: "#b3a1b0", accent: "#c66f9d", secondary: "#72aaa1", opacity: 0.98, density: "comfortable" },
    maelstrom: { label: "Maelstrom — Красный импульс", group: "gang", background: "#171315", panel: "#281d21", text: "#eee6e8", muted: "#aa9ca0", accent: "#bf5d68", secondary: "#7b8d99", opacity: 0.98, density: "comfortable" },
    valentinos: { label: "Valentinos — Золото и вино", group: "gang", background: "#1c1715", panel: "#2d2521", text: "#f0e9df", muted: "#b0a397", accent: "#c49a5d", secondary: "#a76968", opacity: 0.98, density: "comfortable" },
    animals: { label: "Animals — Бетон и лайм", group: "gang", background: "#171a16", panel: "#262b23", text: "#e9ede5", muted: "#a5ac9d", accent: "#93a85d", secondary: "#b88b6b", opacity: 0.98, density: "comfortable" },
    sixthstreet: { label: "6th Street — Пыльный флаг", group: "gang", background: "#17191a", panel: "#252a2c", text: "#e9eceb", muted: "#a2aaab", accent: "#8b9fa6", secondary: "#b46d63", opacity: 0.98, density: "comfortable" },
    voodooboys: { label: "Voodoo Boys — Тихий фиолет", group: "gang", background: "#15131c", panel: "#24202f", text: "#ebe8f1", muted: "#a59fb2", accent: "#8e76ae", secondary: "#6f9d99", opacity: 0.98, density: "comfortable" },
    mox: { label: "Mox — Мягкая фуксия", group: "gang", background: "#1b151c", panel: "#2c222d", text: "#f1e8f0", muted: "#b2a3b0", accent: "#c47ba7", secondary: "#7fa1a3", opacity: 0.98, density: "comfortable" },
    scavengers: { label: "Scavengers — Холодный ржавый", group: "gang", background: "#191716", panel: "#282421", text: "#ece8e2", muted: "#aaa29a", accent: "#a77962", secondary: "#75949b", opacity: 0.98, density: "comfortable" },
    wraiths: { label: "Wraiths — Пыль и кровь", group: "gang", background: "#1d1615", panel: "#2d2220", text: "#efe7e2", muted: "#b0a09a", accent: "#b7655d", secondary: "#a38b62", opacity: 0.98, density: "comfortable" },
    aldecaldos: { label: "Aldecaldos — Пустынный горизонт", group: "gang", background: "#201b15", panel: "#30291f", text: "#efe9df", muted: "#b0a695", accent: "#b28c5e", secondary: "#6f9693", opacity: 0.98, density: "comfortable" },

    traumateam: { label: "Trauma Team — Медицинский бирюзовый", group: "city", background: "#111b1b", panel: "#1c2929", text: "#e7eeee", muted: "#9faaaa", accent: "#67a59e", secondary: "#c1a266", opacity: 0.98, density: "comfortable" },
    maxtac: { label: "MaxTac — Ночной перехват", group: "city", background: "#12161d", panel: "#1e2631", text: "#e6ebf1", muted: "#9da7b3", accent: "#6d8db5", secondary: "#a05d68", opacity: 0.98, density: "comfortable" },
    ncpd: { label: "NCPD — Патрульный синий", group: "city", background: "#141a20", panel: "#222b34", text: "#e8edf0", muted: "#a0a9af", accent: "#7598b3", secondary: "#b58a63", opacity: 0.98, density: "comfortable" },
    netwatch: { label: "NetWatch — Сетевой индиго", group: "city", background: "#141522", panel: "#22243a", text: "#e9e9f1", muted: "#a2a2b4", accent: "#7c82b5", secondary: "#6fa09c", opacity: 0.98, density: "comfortable" },
    afterlife: { label: "Afterlife — Тёмный бар", group: "city", background: "#171816", panel: "#252824", text: "#eceae4", muted: "#aaa99f", accent: "#9a9b70", secondary: "#8c7b9b", opacity: 0.98, density: "comfortable" },
    nightmarket: { label: "Ночной рынок — Тёплый неон", group: "city", background: "#1b1718", panel: "#2b2426", text: "#f0e8e5", muted: "#b0a09f", accent: "#b97b68", secondary: "#799b9e", opacity: 0.98, density: "comfortable" }
  };
  ...
  const windowKey = `${localKey}:cyber-archive-view-window`;
    ...
    for ( const notebook of Object.values(data.notebooks) ) {
      ...
      for ( const list of Object.values(notebook.entries) ) for ( const entry of list ) {
        ...
        if ( entry.type === "people" ) {
          ...
          const legacyTypes = Array.isArray(entry.contactTypes) ? [...entry.contactTypes] : [];
          if ( entry.contactType ) legacyTypes.push(String(entry.contactType));
          let cleanedTypes = [...new Set(legacyTypes.map(value => String(value || "")).filter(value => validContactTypes.has(value)))];
          ...
          if ( Number(entry.contactModelVersion || 0) < CONTACT_MODEL_VERSION ) {
            // v7.0.0–7.0.3 смешивали «кем является персонаж» и «с кем он связан».
            ...
            for ( const type of ["lawman", "noosphere", "nomad"] ) {
              const field = CONTACT_TYPE_META[type].legacyField;
            ...
            for ( const type of ["gang", "corporate"] ) {
              ...
              else cleanedTypes = cleanedTypes.filter(item => item !== type); // старый автотег от поля связи
            }
          ...
          entry.contactType = entry.contactTypes[0] || ""; // legacy mirror for older exports/macросов
          entry.lastSeen ??= "";
  ...
  function bookAppearance(book = notebook()) {
    ...
    const appearance = normalizeAppearance(readArchiveAppearance(book, "cyber"));
    writeArchiveAppearance(book, "cyber", appearance);
    return appearance;
  }
  ...
  function syncLegacyContactType(person) {
  ...
  function dashboard(book) {
    ...
    return `<div class="pcm-welcome"><div><span>NIGHT CITY // ПОЛЕВОЙ АРХИВ</span><h1>${esc(book.actorName)}</h1><p>Контакты и фракции: быстро добавить человека, назначить несколько категорий и сразу привязать к конкретной группе — без метаинформации из листов NPC.</p></div><img src="${esc(book.actorImg)}" alt=""></div>
      <section class="pcm-capture pcm-capture-simple"><div><small>АКТИВНАЯ СЦЕНА · ${esc(sceneName)}</small><h2>${selectedCount ? `Запомнить выбранных: ${selectedCount}` : "Запомнить текущую точку"}</h2><p>${selectedCount ? "Будут сохранены только имя, арт и место встречи." : "Будет сохранён снимок только той области сцены, которую реально видит выбранный токен."}</p><div class="pcm-scene-status"><span class="${sceneLocation ? "known" : "new"}">${sceneLocation ? `⌖ Уже записано: ${esc(sceneLocation.title)}` : "⌖ Точка ещё не записана"}</span><span><b>Ctrl+Shift+M</b></span></div></div><div class="pcm-capture-actions"><button class="primary" data-action="remember-context">${selectedCount ? "◉ Запомнить контакты" : "⌖ Запомнить точку"}</button><button data-action="open-chat">✉ Написать контакту</button></div></section>
      ${selectedCount ? `<section class="pcm-scene-context"><header><div><h2>Выбранные токены</h2><p>До импорта видно, какие контакты уже сохранены.</p></div><small>${selectedCount > 6 ? `Показано 6 из ${selectedCount}` : `${selectedCount} выбрано`}</small></header><div class="pcm-scene-token-grid">${tokenCards}</div></section>` : ""}
      ${inboxCount ? `<section class="pcm-inbox-banner"><div><b>${inboxCount}</b><span><strong>Разобрать после игры</strong><small>Быстрые записи сохранены и ждут уточнения.</small></span></div><button data-action="nav" data-section="inbox">Открыть →</button></section>` : ""}
      ${book.entries.subscriptions.length ? `<section class="pcm-dashboard-subscriptions"><header><div><small>SERVICE STATUS</small><h2>Активные подписки</h2></div><button data-action="nav" data-section="subscriptions">Все подписки →</button></header><div>${sortedEntries(book.entries.subscriptions).slice(0,3).map(entry => subscriptionCard(entry, true)).join("")}</div></section>` : ""}
      <section class="pcm-quick pcm-quick-simple"><div><h2>Быстрая запись</h2><p><b>Ctrl+Enter</b> — сохранить, не отвлекаясь от сцены.</p></div><textarea data-quick data-autogrow placeholder="Фраза, адрес, слух, решение, долг…">${esc(state.quick)}</textarea><div class="pcm-quick-save"><select data-quick-type>${[["notes", "Заметка"], ["clues", "Зацепка"], ["quests", "Заказ"], ["people", "Контакт"], ["gangs", "Банда"], ["corporations", "Корпорация"], ["fixers", "Фиксер"], ["rippers", "Рипер"], ["locations", "Точка"]].map(([key, label]) => opt(key, state.quickType, label)).join("")}</select><button class="primary" data-action="quick-save">Сохранить</button></div></section>
      <div class="pcm-stat-grid pcm-stat-grid-simple">${mainSections.map(key => `<button data-action="nav" data-section="${key}"><b>${counts[key]}</b><span>${SECTIONS[key].icon} ${SECTIONS[key].label}</span></button>`).join("")}</div>
      <section class="pcm-recent"><header><h2>Недавнее</h2><button data-action="global-search">⌕ Найти</button></header>${recent.length ? recent.map(entry => `<button data-action="open-entry" data-section="${entry.type}" data-entry-id="${entry.id}"><b>${SECTIONS[entry.type].icon}</b><span>${esc(recordTitle(entry))}${recordHeadline(entry) ? `<em class="pcm-mini-headline">${esc(recordHeadline(entry))}</em>` : ""}<small>${short((DIRECTORY_TYPES.has(entry.type) ? entry.quickNotes : "") || entry.summary || entry.content, 70)}</small></span></button>`).join("") : '<p class="muted">Архив пока пуст.</p>'}</section>`;
  }
  ...
  function directoryListView(book, key) {
    ...
    const content = blocks.length
      ...
      : `<div class="pcm-empty"><b>${meta.icon}</b><h2>${esc(meta.emptyTitle)}</h2><p>Создайте группу или сразу добавьте выбранного персонажа. Если у контакта уже есть старая привязка, она появится здесь автоматически.</p><button class="primary" data-action="quick-create-directory" data-section="${key}">+ Создать</button></div>`;
  ...
  function personListCard(person, book, { unlinkDirectory = null } = {}) {
    ...
    const affiliations = personCompactTags(person, { limit: 6 });
  ...
  function entryTypeReadPanels(book, entry) {
    ...
    if ( entry.type === "gangs" ) return `${detailValue("Опасность", entry.danger)}${detailValue("Отношение к нам", entry.attitude)}${detailValue("Агрессивность", entry.aggression)}`;
    if ( entry.type === "corporations" ) return `${detailValue("Угроза", entry.danger)}${detailValue("Отношение к нам", entry.attitude)}${detailValue("Текущая позиция", entry.posture)}${detailValue("Публичная маска", entry.publicFace, {wide:true})}`;
    if ( entry.type === "fixers" ) return `${detailValue("Район работы", entry.district)}${detailValue("Репутация", entry.reputation)}${detailValue("Отношение к нам", entry.attitude)}${detailValue("Текущие дела", entry.currentDeals, {wide:true})}`;
    if ( entry.type === "rippers" ) return `${detailValue("Клиника / район", entry.clinic)}${detailValue("Специализация", entry.specialty)}${detailValue("Доверие", entry.trust)}${detailValue("Отношение к нам", entry.attitude)}`;
    if ( entry.type === "subscriptions" ) return `${detailValue("Сервис", entry.provider)}${detailValue("Тариф", entry.plan)}${detailValue("Цена", [entry.price, entry.currency].filter(Boolean).join(" "))}${detailValue("Срок", `${subscriptionTerm(entry)} дней`)}${detailValue("Осталось", `${subscriptionDays(entry)} дней`)}${detailValue("Статус", entry.status)}${detailValue("Условия продления", entry.renewalNote, {wide:true})}`;
    if ( entry.type === "quests" ) {
      ...
      return `${detailValue("Статус", entry.status)}${detailValue("Заказчик", giver?.title || "")}${detailValue("Дедлайн", entry.deadline)}${detailValue("Что нужно сделать", entry.objective, {wide:true})}${detailValue("Оплата / выгода", entry.reward, {wide:true})}${detailValue("Следующий шаг", entry.nextStep, {wide:true})}${tasks}${locationPanel}`;
    }
    if ( entry.type === "clues" ) {
    ...
    if ( entry.type === "books" ) {
      ...
      return `${detailValue("Состояние", entry.status)}${detailValue("Источник файла", entry.author)}${detailValue("Язык / формат", entry.language)}${detailValue("Кодировка", entry.script)}${detailValue("Защита / шифр", entry.cipher)}${detailValue("Помогает", helper?.title || "")}${detailValue("Прогресс", `${clampPercent(entry.decodingProgress)}% · ${decodingPhase(entry.decodingProgress).label}`)}${detailValue("Ключи и доступ", entry.decodingKey, {wide:true})}${detailValue("Метод взлома", entry.method, {wide:true})}${detailValue("Что уже извлечено", entry.discoveries, {wide:true})}${detailValue("Следующий шаг", entry.nextStep, {wide:true})}${stages}${locationPanel}`;
    }
    if ( entry.type === "sessions" ) return `${detailValue("Дата игры", entry.realDate)}${detailValue("Дата в мире", entry.gameDate)}${detailValue("Участники", entry.participants, {wide:true})}${detailValue("Что произошло", entry.events, {wide:true})}${detailValue("Решения и последствия", entry.decisions, {wide:true})}${detailValue("Добыча и расходы", entry.loot, {wide:true})}${detailValue("К следующей игре", entry.nextTime, {wide:true})}${locationPanel}`;
    if ( entry.type === "notes" ) return `${detailValue("Категория", entry.category)}${locationPanel}`;
  ...
  function entryOverview(book, entry) {
    ...
    const image = entry.image ? `<div class="pcm-generic-hero-image"><img src="${esc(entry.image)}" alt=""></div>` : `<div class="pcm-generic-hero-image placeholder">${section?.icon || "▧"}</div>`;
  ...
  function themePanel(book) {
    ...
    const theme = bookAppearance(book);
    const currentGroup = THEME_PRESETS[theme.preset]?.group ?? "calm";
    ...
    return `<div class="pcm-modal-backdrop"><section class="pcm-theme-panel" role="dialog" aria-label="Оформление архива"><header><div><small>NIGHT CITY // 30 ПАЛИТР</small><h2>Внешний вид</h2><p>От спокойных рабочих тем до палитр корпораций, банд и городских служб. Фон текста остаётся приглушённым даже у ярких пресетов.</p></div><button data-action="close-appearance">×</button></header>
      <div class="pcm-theme-groups">${groups}</div>
      <div class="pcm-interface-controls"><label><span>Оболочка</span><select data-theme-field="shell">${opt("datapad", theme.shell, "DATAPAD — металлический планшет")}${opt("flat", theme.shell, "Плоский интерфейс")}</select><small>DATAPAD добавляет физический корпус и утопленный экран, не меняя структуру данных.</small></label><label><span>Подсветка устройства</span><select data-theme-field="effects">${opt("off", theme.effects, "Выключена")}${opt("soft", theme.effects, "Мягкая")}${opt("vivid", theme.effects, "Выразительная")}</select><small>Только статичная подсветка корпуса и индикаторов. Бегущих полос и анимации экрана больше нет.</small></label></div>
    ...
    return `<div class="pcm-modal-backdrop"><section class="pcm-help-panel" role="dialog" aria-label="FAQ и инструкция"><header><div><small>NC://DATAPAD // FAQ</small><h2>Инструкция по каждому разделу</h2><p>Главное правило архива: существующую запись сначала читаем. Редактор открывается только по явной кнопке «Редактировать».</p></div><button data-action="close-help">×</button></header>
      ...
      <div class="pcm-help-columns">
        <section><h3>Люди и силы</h3>
          ${faq("◉ Контакты", `<p><b>Что здесь:</b> только люди без категории. Клик по карточке всегда открывает досье.</p><p><b>Категории:</b> контакт может иметь несколько одновременно: например Банда + Корпорат + Законник. Добавление новой категории не удаляет старые и не создаёт копию человека.</p><p><b>Сообщение:</b> конверт на карточке открывает отправку именно по этому контакту.</p>`, true)}
  ...
  function deviceHardware() {
    return `<div class="pcm-device-hardware" aria-hidden="true">
      ...
      <i class="pcm-hw-key hw-key-power"><b>PWR</b></i><i class="pcm-hw-key hw-key-up"><b>+</b></i><i class="pcm-hw-key hw-key-down"><b>−</b></i>
      ...
      <i class="pcm-hw-serial">NIGHT CITY FIELD TERMINAL · NC-DP/70</i>
    </div>`;
  }
  ...
  /* v7.0.4 — категории персонажа отделены от его связей */
  ...
  /* v6.8.1 — адаптивная боковая навигация: текст не выходит за кнопку и не сталкивается со счётчиком. */
  ...
  /* v6.9 — human-first routing, drag & drop, readable FAQ */
  ...
  const root = hostRoot;
  if ( !root ) throw new Error("Кибер-Архив: hostRoot недоступен");
  root.id = "pcm-root";
  root.classList.add("field-archive-embedded", "cyber-archive-view");
...
${EMBEDDED_HOST_CSS}
  ...
  /* v7.0 FINAL — читаемость и фракции без лишних шагов */
  ...
  /* v7.0.2 — замки прямо в досье + карточки, растущие по содержимому */
  ...
  /* v7.0.5 — теги в компактных карточках + заметная привязка существующего контакта */
  ...
  /* v7.0.8 FUNCTIONAL BACKPORT — behavior from later revisions, original 7.0.7 visual language preserved. */
  #pcm-root .pcm-device-screen{contain:layout paint style}
  #pcm-root .pcm-window{position:fixed!important}

  #pcm-root .pcm-contact-toolbar{margin:0 0 10px;padding:10px;display:grid;grid-template-columns:minmax(220px,1.7fr) minmax(130px,.75fr) minmax(130px,.75fr) minmax(150px,.85fr) 70px;gap:8px;align-items:end;background:var(--panel-alpha);border:1px solid var(--line);border-radius:10px}
  #pcm-root .pcm-contact-toolbar>label{display:grid;gap:4px;min-width:0}
  #pcm-root .pcm-contact-toolbar>label>span{color:var(--muted);font-size:9px;font-weight:900;letter-spacing:.08em}
  #pcm-root .pcm-contact-toolbar select,#pcm-root .pcm-contact-searchbox{height:36px;color:var(--ink);background:var(--field);border:1px solid var(--line);border-radius:7px}
  #pcm-root .pcm-contact-toolbar select{width:100%;padding:0 8px}
  #pcm-root .pcm-contact-searchbox{display:grid;grid-template-columns:34px minmax(0,1fr) auto;align-items:center;overflow:hidden}
  #pcm-root .pcm-contact-searchbox>b{display:grid;place-items:center;height:100%;color:var(--teal);border-right:1px solid var(--line)}
  #pcm-root .pcm-contact-searchbox>input{width:100%;height:100%;padding:0 9px;color:var(--ink);background:transparent;border:0;outline:0}
  #pcm-root .pcm-contact-searchbox>i{padding-right:8px;color:var(--muted);font-size:7px;font-style:normal;letter-spacing:.08em;white-space:nowrap}
  #pcm-root .pcm-contact-filter-status{height:52px;display:grid;place-content:center;text-align:center;border-left:1px solid var(--line)}
  #pcm-root .pcm-contact-filter-status>b{color:var(--gold);font-size:20px;line-height:1}
  #pcm-root .pcm-contact-filter-status>span{margin-top:3px;color:var(--muted);font-size:8px}
  #pcm-root .pcm-contact-index-info{margin:0 0 10px;padding:8px 10px;display:flex;gap:9px;align-items:center;color:var(--muted);background:var(--field);border:1px solid var(--line);border-radius:8px;font-size:10px}
  #pcm-root .pcm-contact-index-info>b{color:var(--teal);font-size:8px;letter-spacing:.1em;white-space:nowrap}

  #pcm-root .pcm-directory-unlink{width:100%;min-height:34px;border-width:1px 0 0!important;border-radius:0!important;color:#ec887c;background:transparent!important}
  #pcm-root .pcm-directory-unlink:hover{background:color-mix(in srgb,#ec887c 8%,transparent)!important}

  #pcm-root .pcm-inline-manual-tag-chips{display:flex;flex-wrap:wrap;gap:6px;margin:2px 0 6px}
  #pcm-root .pcm-inline-manual-tag-chips button{min-height:29px!important;padding:4px 8px!important;border-radius:999px!important;font-size:11px!important}
  #pcm-root .pcm-inline-manual-tag-chips button b{margin-left:5px;color:#ec887c}

  #pcm-root .pcm-person-quick-edit{width:100%;padding:12px;display:grid;gap:10px}
  #pcm-root .pcm-quick-edit-section{padding:12px;background:var(--panel-alpha);border:1px solid var(--line);border-radius:10px}
  #pcm-root .pcm-quick-edit-section>header{margin-bottom:9px;padding-bottom:7px;border-bottom:1px solid var(--line)}
  #pcm-root .pcm-quick-edit-section>header small{color:var(--teal);font-size:9px;font-weight:900;letter-spacing:.08em}
  #pcm-root .pcm-quick-edit-section>header h3{margin:2px 0;color:var(--heading);font-size:14px}
  #pcm-root .pcm-quick-edit-section>header p{margin:2px 0 0;color:var(--muted);font-size:10px}
  #pcm-root .pcm-quick-edit-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
  #pcm-root .pcm-quick-edit-grid>label{display:grid;gap:4px;min-width:0}
  #pcm-root .pcm-quick-edit-grid>label.wide{grid-column:1/-1}
  #pcm-root .pcm-quick-edit-grid>label>span{color:var(--muted);font-size:10px;font-weight:800}
  #pcm-root .pcm-quick-edit-grid input,#pcm-root .pcm-quick-edit-grid select,#pcm-root .pcm-quick-edit-grid textarea{width:100%;color:var(--ink);background:var(--field);border:1px solid var(--line);border-radius:7px;outline:0}
  #pcm-root .pcm-quick-edit-grid input,#pcm-root .pcm-quick-edit-grid select{height:35px;padding:0 8px}
  #pcm-root .pcm-quick-edit-grid textarea{min-height:76px;padding:8px;resize:vertical;line-height:1.45}
  #pcm-root .pcm-quick-image-path{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:6px}
  #pcm-root .pcm-quick-location-links .pcm-location-checks{max-height:180px}

  /* Menus are fixed to the viewport so a small macro window cannot clip them. */
  #pcm-root .pcm-context-menu-surface{position:fixed;z-index:1000015;width:min(340px,calc(100vw - 16px));max-height:calc(100vh - 16px);overflow:auto;padding:8px;pointer-events:auto!important;color:var(--ink);background:var(--bg-alpha);border:1px solid var(--line);border-radius:10px;box-shadow:0 18px 50px #000b}
  #pcm-root .pcm-context-menu-surface>header{display:grid;grid-template-columns:42px minmax(0,1fr) 32px;gap:8px;align-items:center;padding-bottom:7px;border-bottom:1px solid var(--line)}
  #pcm-root .pcm-context-menu-surface>header>span:nth-child(2){min-width:0}
  #pcm-root .pcm-context-menu-surface>header small,#pcm-root .pcm-context-menu-surface>header b{display:block}
  #pcm-root .pcm-context-menu-surface>header small{color:var(--muted);font-size:8px;letter-spacing:.06em}
  #pcm-root .pcm-context-menu-surface>header b{margin-top:2px;color:var(--heading);font-size:13px;overflow-wrap:anywhere}
  #pcm-root .pcm-context-menu-surface>header>button{width:32px;min-height:32px;padding:0;font-size:16px}
  #pcm-root .pcm-context-avatar{width:42px;height:42px;display:grid;place-items:center;overflow:hidden;color:var(--gold);background:var(--field);border:1px solid var(--line);border-radius:8px}
  #pcm-root .pcm-context-avatar img{width:100%;height:100%;object-fit:cover}
  #pcm-root .pcm-context-actions,#pcm-root .pcm-context-chip-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5px;margin-top:7px}
  #pcm-root .pcm-context-actions.minor{grid-template-columns:1fr}
  #pcm-root .pcm-context-menu-surface section{margin-top:8px;padding-top:7px;border-top:1px solid #ffffff10}
  #pcm-root .pcm-context-menu-surface section>small{display:block;margin-bottom:5px;color:var(--teal);font-size:8px;font-weight:900;letter-spacing:.08em}
  #pcm-root .pcm-context-menu-surface button{min-height:34px;padding:5px 8px;font-size:10.5px;line-height:1.2}
  #pcm-root .pcm-context-chip-grid button.active{color:var(--heading);background:var(--accent-soft);border-color:var(--gold);box-shadow:inset 0 0 0 1px var(--gold)}
  #pcm-root .pcm-context-chip-grid.roles button.active:hover{color:#ffb0a8;border-color:#d46b62;box-shadow:inset 0 0 0 1px #d46b62}
  #pcm-root .pcm-context-role-hint{display:block;margin-top:5px;color:var(--muted);font-size:8px}
  #pcm-root .pcm-context-tag-list{display:flex;flex-wrap:wrap;gap:5px}
  #pcm-root .pcm-context-tag-list button{min-height:29px;padding:4px 7px;border-radius:999px}
  #pcm-root .pcm-context-tag-list button b{margin-left:4px;color:#ec887c}
  #pcm-root .pcm-context-tag-editor{display:grid;grid-template-columns:minmax(0,1fr) auto 32px;gap:5px;align-items:end}
  #pcm-root .pcm-context-tag-editor label{display:grid;gap:3px}
  #pcm-root .pcm-context-tag-editor label>span{color:var(--muted);font-size:8px;font-weight:900}
  #pcm-root .pcm-context-tag-editor input{width:100%;height:34px;padding:0 8px;color:var(--ink);background:var(--field);border:1px solid var(--line);border-radius:7px;outline:0}
  #pcm-root .pcm-context-delete{width:100%;margin-top:8px;color:#ec887c!important}

  @media(max-width:900px){#pcm-root .pcm-contact-toolbar{grid-template-columns:repeat(2,minmax(0,1fr))}#pcm-root .pcm-contact-filter-status{border-left:0;border-top:1px solid var(--line);grid-column:1/-1;height:42px}}
  @media(max-width:620px){#pcm-root .pcm-contact-toolbar,#pcm-root .pcm-quick-edit-grid{grid-template-columns:1fr}#pcm-root .pcm-contact-filter-status,#pcm-root .pcm-quick-edit-grid>label.wide{grid-column:auto}#pcm-root .pcm-context-actions,#pcm-root .pcm-context-chip-grid{grid-template-columns:1fr}}

  /* v7.1.0 — НЕЙРО-СВЯЗЬ + GM workspace */
...
${EMBEDDED_HOST_CSS}
  ...
  root.addEventListener("input", event => {
    ...
    if ( target.matches("[data-theme-field]") ) {
      ...
      writeArchiveAppearance(notebook(), "cyber", theme);
  ...
  root.addEventListener("click", async event => {
    ...
    if ( action === "context-entry-share" && entry && entry.type !== "people" ) {
      ...
      await openArchiveShareDialog({ senderUser: game.user, sourceOwnerUserId: snapshot.sourceOwnerUserId, sourceActor: snapshot.sourceActor, scope: "entry", label: recordTitle(entry), records: [{ section: entry.type, entry: clone(entry) }], themeSource: state.root, archiveMode: "cyber" });
      return;
    }
    if ( action === "context-share-person" && contextPerson ) {
      const snapshot = getShareSnapshot();
      closeContactContextMenu();
      await openArchiveShareDialog({ senderUser: game.user, sourceOwnerUserId: snapshot.sourceOwnerUserId, sourceActor: snapshot.sourceActor, scope: "entry", label: contextPerson.title || "Контакт", records: [{ section: "people", entry: clone(contextPerson) }], themeSource: state.root, archiveMode: "cyber" });
      return;
    }
    ...
    if ( action === "theme-preset" && THEME_PRESETS[button.dataset.preset] ) {
      ...
      writeArchiveAppearance(notebook(), "cyber", normalizeAppearance({ ...THEME_PRESETS[button.dataset.preset], preset: button.dataset.preset, fontSize: current.fontSize, opacity: THEME_PRESETS[button.dataset.preset].opacity, density: current.density, shell: current.shell, effects: current.effects }));
      dirty(); render(); return;
    }
    if ( action === "theme-reset" ) { writeArchiveAppearance(notebook(), "cyber", normalizeAppearance()); dirty(); render(); return; }
