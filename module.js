...
const NETRUNNER_SCAN_UUID =
  `Compendium.${MODULE_ID}.cyberpunk-items.Item.${NETRUNNER_SCAN_ID}`;
...
function enqueueActor(actor, operation) {
  ...
  const previous = actorQueues.get(actor.id) ?? Promise.resolve();
  const next = previous
    .catch(() => undefined)
    .then(operation)
    .catch((error) => {
    ...
    .finally(() => {
      if (actorQueues.get(actor.id) === next) actorQueues.delete(actor.id);
    });
  actorQueues.set(actor.id, next);
  return next;
}
...
function onRenderSheet(app, html) {
  ...
  const element = ElementClass && html instanceof ElementClass
    ? html
    : html?.[0];
...
function refreshCyberwareRuleSettings({ reconcileHumanity = false } = {}) {
  const actors = globalThis.game?.actors?.filter?.(
...
function registerRuleSettings() {
  ...
  game.settings.register(MODULE_ID, "hardCostMultiplier", {
    ...
    onChange: () =>
      refreshCyberwareRuleSettings({ reconcileHumanity: true }),
  });
}
...
Hooks.on("preCreateItem", (item, _data, options, userId) => {
  ...
  options ??= {};
  const pending = localOperationState(
    options,
    PENDING_CREATE_ITEMS,
    () => [],
  );
...
Hooks.on("preUpdateItem", (item, changes, options) => {
  ...
  if (!requestedInstalled && wasInstalled) {
    const validation = CyberwareTab.removalValidation(
      validationActor,
      item,
    );
    if (validation) {
...
Hooks.on("createItem", (item, options, userId) => {
  if (!isActingUser(userId)) return;
  if (item.pack === `${MODULE_ID}.cyberpunk-items`) {
...
async function removeEmbeddedNetrunnerInterfaceScan(actor) {
  const embeddedActions = [...actor.items].filter((item) =>
...
export async function migrateActor(actor) {
  ...
  const hasInstalledBiosystem = itemState.some(
    (state) =>
      state.installed &&
      CyberwareTab.isPktBiosystem(state.item),
  );
  ...
  const installedAccelerators = itemState.filter(
    (state) =>
      ...
      (
        ...
        state.item.system?.traits?.value?.includes?.(
          "neironn-uskoritell",
        )
      ),
  );
  ...
  for (const state of itemState) {
    ...
    if (!CyberwareTab.isCyberware(item) &&
    ...
    if (descriptionMetadata.hardCost !== null) describedFlagKeys.add("hardCost");
  ...
  await CyberwareTab.reconcileHumanity(actor);
  await CyberwareTab.reconcileGrantedItems(actor);
  normalized.netrunnerActionsRemoved =
